...
import { Shield, Server, Database, Network, Lock, Eye, AlertTriangle, Zap, Cloud, HardDrive, Cpu, Monitor, RefreshCw, CheckCircle, ArrowRight } from 'lucide-react';
...
const InfrastructureDesign = () => {
  ...
  const infrastructureComponents = {
    ...
    'dns-lb': {
      ...
      description: 'DNS resolution with global load balancing and health checking',
      technologies: ['F5 DNS', 'Global Traffic Manager', 'Health Monitors'],
      specifications: ['Multi-datacenter failover', 'Geographic routing', 'Health-based routing', 'SSL termination'],
    ...
    'waf': {
      ...
      specifications: ['OWASP Top 10 protection', 'API security', 'Rate limiting', 'Geo-blocking'],
      redundancy: 'HA pair with session sync',
      category: 'security'
    },
    'fw-external': {
      name: 'External Firewall',
      description: 'Perimeter security firewall protecting against external threats',
      technologies: ['Fortinet FortiGate', 'IPS/IDS', 'SSL Inspection'],
      specifications: ['Stateful inspection', 'Application control', 'VPN termination', 'Threat intelligence'],
    ...
    'fw-internal': {
      ...
      specifications: ['Zone-based filtering', 'User-ID integration', 'Application visibility', 'Threat prevention'],
    ...
    'lb-app': {
      ...
      specifications: ['SSL offloading', 'Session persistence', 'Advanced health monitoring', 'iRules automation'],
    ...
    'lb-db': {
      ...
      specifications: ['Read/write splitting', 'Connection pooling', 'Query routing', 'Failover automation'],
    ...
    'apigee-gw': {
      ...
      description: 'API management gateway providing secure API access and management',
      technologies: ['Google APIGEE Edge', 'OAuth 2.0', 'Rate Limiting'],
      specifications: ['4 nodes minimum', '16 vCPU', '32GB RAM', 'SSD storage', 'Auto-scaling'],
    ...
    'reverse-proxy': {
      ...
      specifications: ['Load balancing', 'SSL acceleration', 'HTTP/2 support', 'Compression'],
    ...
    'keycloak': {
      ...
      description: 'Identity and access management cluster with high availability',
      technologies: ['Keycloak 22+', 'JGroups clustering', 'PostgreSQL backend'],
      specifications: ['3 node cluster', '8 vCPU', '16GB RAM', 'Shared database', 'Session replication'],
    ...
    'fhir-server': {
      ...
      specifications: ['6 nodes minimum', '16 vCPU', '32GB RAM', 'SSD storage', 'JVM tuning'],
    ...
    'converter': {
      ...
      technologies: ['HAPI FHIR Converter', 'Custom mappers', 'Validation engine'],
      specifications: ['4 nodes', '8 vCPU', '16GB RAM', 'High CPU for transformation'],
      redundancy: 'Horizontal scaling with load balancing',
      category: 'application'
    },
    'notification-producer': {
      name: 'Notification Producer',
      description: 'Service that produces lightweight notifications about FHIR resource changes',
      technologies: ['Event Sourcing', 'Redpanda Producer', 'Resource Change Detection'],
      specifications: ['3 node cluster', '4 vCPU', '8GB RAM', 'High throughput', 'Low latency'],
    ...
    'sehhaty-be': {
      ...
      description: 'Healthcare provider backend application cluster with FHIR client capabilities',
      technologies: ['Spring Boot', 'Microservices', 'HAPI FHIR Client', 'Container runtime'],
      specifications: ['Auto-scaling group', '4-12 instances', '8 vCPU', '16GB RAM'],
    ...
    'redpanda': {
      ...
      description: 'High-performance streaming platform for event-driven architecture',
      technologies: ['Redpanda', 'Raft consensus', 'Tiered storage'],
      specifications: ['6 broker minimum', '16 vCPU', '64GB RAM', 'NVMe SSD', '10Gbps network'],
    ...
    'schema-registry': {
      ...
      description: 'Schema evolution and compatibility management for streaming',
      technologies: ['Schema Registry', 'Avro schemas', 'Compatibility checks'],
      specifications: ['3 node cluster', '4 vCPU', '8GB RAM', 'High availability'],
      redundancy: 'Leader election with automatic failover',
      category: 'streaming'
    },
    'streaming-consumers': {
      name: 'Streaming Consumers',
      description: 'Consumers that process events from Redpanda for FHIR resource creation',
      technologies: ['Redpanda Consumers', 'Stream Processing', 'Event Handling'],
    ...
    'redis-cluster': {
      ...
      specifications: ['6 nodes minimum', '8 vCPU', '64GB RAM', 'Network-attached SSD'],
    ...
    'postgresql': {
      ...
      specifications: ['Primary + 2 replicas', '16 vCPU', '128GB RAM', 'High-IOPS SSD'],
    ...
    'mssql': {
      ...
      specifications: ['Primary + 2 replicas', '16 vCPU', '128GB RAM', 'SSD storage'],
    ...
    'minio': {
      ...
      specifications: ['8 node minimum', '8 vCPU', '32GB RAM', 'High-capacity drives'],
    ...
    'monitoring': {
      ...
      specifications: ['Time-series database', 'Log aggregation', 'Custom dashboards', 'Alert routing'],
    ...
    'siem': {
      ...
      technologies: ['Splunk Enterprise', 'Security orchestration', 'Threat intelligence'],
      specifications: ['Indexer cluster', 'Search head cluster', 'Heavy forwarders'],
  ...
  const securityZones = [
    ...
    },
    {
      ...
      components: ['keycloak', 'fhir-server', 'converter', 'notification-producer', 'sehhaty-be']
    },
    {
      id: 'streaming',
      name: 'Streaming Zone',
  ...
  const eventFlowSteps = [
    ...
    },
    {
      ...
      zone: 'NPHIES',
      components: ['Validation Engine', 'Integration Engine', 'Streaming Producer'],
      critical: true
    },
    {
        ...
        <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-96 overflow-y-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <h3 className="font-semibold mb-3 mt-4 text-gray-800">Redundancy Model</h3>
              <div className="bg-blue-50 p-3 rounded border">
                <span className="text-sm font-medium text-blue-800">{component.redundancy}</span>
              </div>
            </div>
  ...
  const EventFlowDiagram = () => (
    <div className="mt-8 p-6 bg-white rounded-lg border-2 border-blue-200">
      <h3 className="text-lg font-bold mb-4 text-gray-800">Event-Driven Infrastructure Flow</h3>
              ...
              <React.Fragment key={index}>
                {index > 0 && (
                  <div className="flex justify-center">
                    <ArrowRight size={20} className="transform rotate-90 text-gray-400" />
                  </div>
                )}

                <div className="flex items-center">
                  <div className="w-1/6 text-center">
                    <div className={`rounded-full h-16 w-16 bg-${color}-100 flex items-center justify-center mx-auto`}>
                      {step.zone === 'External' && <Server className={`text-${color}-600`} size={32} />}
                      {step.zone === 'NPHIES' && <GitBranch className={`text-${color}-600`} size={32} />}
                      {step.zone === 'Streaming' && <Activity className={`text-${color}-600`} size={32} />}
                      {step.zone === 'Application' && <Database className={`text-${color}-600`} size={32} />}
                      {step.zone === 'Sehhaty' && <Smartphone className={`text-${color}-600`} size={32} />}
                      {step.zone === 'DMZ / Application' && <Shield className={`text-${color}-600`} size={32} />}
                    </div>
                    <p className="text-sm font-medium mt-1">{index + 1}. {step.step}</p>
                  </div>
                    ...
                    <div className={`p-3 bg-${color}-50 rounded-lg border border-${color}-200`}>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {step.components.map((component, compIndex) => (
                          <span key={compIndex} className={`inline-block px-2 py-1 text-xs bg-${color}-100 text-${color}-800 rounded`}>
                            {component}
                          </span>
                        ))}
                      </div>
                      {step.critical && (
                        <div className="mt-2 flex items-center">
                          ...
                          <span className="text-xs text-orange-700">Infrastructure Resilience Critical</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
        <p className="text-sm font-medium text-green-800">
          The infrastructure is designed to support the event-driven architecture pattern, ensuring high availability, fault tolerance, and 
          scalability at each step of the data flow. Circuit breakers and retry mechanisms protect the system from cascading failures, 
          while redundant components eliminate single points of failure.
        </p>
      </div>
    </div>
  );
    ...
    <div className="w-full min-h-screen bg-gray-50 p-6">
      <header className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Enterprise-grade infrastructure with comprehensive security and high availability
        </p>
      </header>
      ...
      {expandedSections.network && (
        <div className="mb-8 p-6 bg-white rounded-lg border shadow-sm">
          ...
          {/* Network Infrastructure Components */}
                ...
                {/* Application Zone */}
                <SectionHeader title="Application Zone Architecture" id="app" icon={Server} />
                ...
                {/* Streaming Zone */}
                <SectionHeader title="Streaming Zone Architecture" id="streaming" icon={Zap} />
                ...
                {/* Monitoring Zone */}
                <SectionHeader title="Monitoring Zone Architecture" id="monitoring" icon={Monitor} />
                ...
                <SectionHeader title="Circuit Breaker Patterns" id="resilience" icon={AlertTriangle} />
                {expandedSections.resilience && (
                  <div className="mb-8 p-6 bg-white rounded-lg border shadow-sm">
                    <p className="text-sm text-gray-700 mb-4">
                      Circuit breakers are implemented to prevent cascading failures and maintain system stability during partial outages.
                      The following patterns ensure that temporary failures in one component don't propagate throughout the system.
                    </p>
                        ...
                        <tbody>
                          {circuitBreakerStrategies.map((strategy, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                              ...
                              <td className="border p-3 text-sm font-mono">{strategy.thresholds}</td>
                    ...
                    <p className="text-sm text-gray-700 mb-4">
                      Complementing the circuit breakers, these retry strategies ensure temporary failures can be automatically recovered
                      without manual intervention, improving the system's overall resilience.
                    </p>
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse">
                        <thead>
                        ...
                        <tbody>
                          {retryStrategies.map((strategy, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                              <td className="border p-3 font-medium">{strategy.component}</td>
                              <td className="border p-3 text-sm">{strategy.strategy}</td>
                              <td className="border p-3 text-sm font-mono">{strategy.config}</td>
                ...
                <SectionHeader title="Infrastructure Best Practices" id="bestPractices" icon={CheckCircle} />
                {expandedSections.bestPractices && (
                  <div className="mb-8 p-6 bg-white rounded-lg border shadow-sm">
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        <h3 className="font-semibold text-lg mb-3 text-blue-600">High Availability</h3>
                      ...
                      <div>
                        <h3 className="font-semibold text-lg mb-3 text-green-600">Security</h3>
                        <ul className="text-sm text-gray-700 space-y-2">
                      ...
                      <div>
                        <h3 className="font-semibold text-lg mb-3 text-purple-600">Performance</h3>
                        <ul className="text-sm text-gray-700 space-y-2">
                      ...
                      <div>
                        <h3 className="font-semibold text-lg mb-3 text-orange-600">Operational Excellence</h3>
                        <ul className="text-sm text-gray-700 space-y-2">
                    ...
                    <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <h4 className="font-semibold text-blue-800 mb-2">Event-Driven Architecture Considerations</h4>
                      <p className="text-sm text-blue-700">
                        The infrastructure is specifically designed to support the event-driven architecture pattern between NPHIES, FHIR, and Sehhaty components. 
                        The Redpanda streaming platform provides the backbone for real-time event processing, while circuit breakers and retry mechanisms ensure 
                        resilience throughout the event flow. This design addresses the data freshness issues identified in the current architecture by enabling 
                        immediate notification and efficient resource retrieval.
                      </p>
                    </div>
                  </div>
                )}
